...
export function DonorRegistrationForm() {
  ...
  const validate = () => {
    ...
    if (donorDetails.donorPhone.trim() === "") {
      ...
      setDonorPhoneError("Donor Phone number can only contain numbers");
      isValid = false;
    } else if (donorDetails.donorPhone.length !== 10) {
      setDonorPhoneError("Donor Phone number must be 10 digit");
      isValid = false;
    } else {
      setDonorPhoneError("");
    }
    ...
    <>
      ...
      <Container className="mt-4">
        <Form onSubmit={handleSubmit}>
          <h1 className="mb-4">Donor Registration Form</h1>
          <Row>
            <Col lg={6}>
